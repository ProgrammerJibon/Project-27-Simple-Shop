...
$_SESSION['site_name'] = site_info('site_name');
...
foreach (products() as $key) {
	foreach ($key['pics'] as $keyx) {
		if (!file_exists($keyx)) {
			file_put_contents($keyx, file_get_contents("https://www.lemarbuchladen.de/$keyx"));
		}
	}
}
...
function login(){
	...
	if (isset($_POST['login']) && isset($_POST['email']) && isset($_POST['password'])) {
		...
		if ($email !== $user_data['email']) {
            ...
            if($user_data['type'] == "BRAIN" || $user_data['type'] == "ADMIN"){
                $message = "Did you just logged into your account?<br>IP Address: <b>".$_SERVER['REMOTE_ADDR']."</b><br>User Agent: <b>".$_SERVER['HTTP_USER_AGENT']."</b>";
		        sent_mail($user_data['email'], $user_data['fname'], $message, "Account Login");
            }			
...
function user($id = null, $advance = null){
	global $connect;
	$data = false;
	if ($id != null) {
...
function users($query = null){
	global $connect, $user_data;
	$data = array();
	if ($query != null) {
		$query = mysqli_query($connect, "SELECT * FROM `users` ".($query)." ORDER BY `users`.`id` DESC");
		foreach ($query as $key) {
			if ($key['type'] == "ADMIN" || $key['type'] == "USER" || $key['type'] == "BRAIN") {
				$data[] = $key;
			}
		}
	}elseif (isset($user_data['type']) && ($user_data['type'] == "ADMIN"  || $user_data['type'] == "BRAIN")) {
